...
def load_select_tables_from_database() -> None:
    ...
    """
    print("Starting to load selected tables...")
    
    try:
        # Create a pipeline with explicit dataset name
        pipeline = dlt.pipeline(
            pipeline_name="rfam",
            destination='duckdb',
            dataset_name="rfam_data",
            full_refresh=True  # This will ensure we start fresh
        )
        ...
        print("Created credentials for database connection")
        
        # Load each table individually with explicit error handling
        tables_to_load = ["family", "clan", "features", "author", "genome"]
        
        for table in tables_to_load:
            try:
                print(f"\nLoading table: {table}")
                source = sql_database(credentials).with_resources(table)
                info = pipeline.run(source, write_disposition="replace", table_name=table)
                print(f"Pipeline run complete for {table}:")
                print(info)
        ...
        import duckdb
        conn = duckdb.connect('rfam.duckdb')
        ...
        for table in tables:
            try:
                ...
                # Show sample data
                sample = conn.execute(f"SELECT * FROM {table[0]} LIMIT 2").fetchall()
